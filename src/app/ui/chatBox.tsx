        ...
        <div className="w-[500px] max-w-full flex flex-col border rounded-lg shadow h-[660px] bg-white dark:bg-zinc-900 px-4">
            <div className="flex-1 overflow-y-auto py-8 space-y-4">
                {messages.map(( message ) => (
                    <div 
                        key={message.id}
                        className={ `max-w-xs md:max-w-sm p-3 rounded-lg shadow text-sm whitespace-pre-wrap ${ messages.role === 'user' ? 'bg-blue-100 text-blue-900' : 'bg-gray-100 text-gray-800'}`}
                    >
